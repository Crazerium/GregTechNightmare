...
public final class CraftingInputMECircuitResolver {
    ...
    public static ItemStack find(Object host, Object patternSlot) {
        ...
        if (config == null) {
            config = findNamedConfig(patternSlot);
        }

        return config == null ? null : one(GTUtility.getIntegratedCircuit(config));
    }
    ...
    public static boolean isIntegratedCircuit(ItemStack stack) {
        ...
        if (exemplar != null && exemplar.getItem() == stack.getItem()) {
            return true;
        }
    ...
    private static ItemStack findNamedStack(Object root) {
        ...
        for (Class<?> type = root.getClass(); type != null && type != Object.class; type = type.getSuperclass()) {
            for (Field field : type.getDeclaredFields()) {
                if (Modifier.isStatic(field.getModifiers()) || !containsCircuitWord(field.getName())) {
                    continue;
                }
    ...
    private static ItemStack findInventoryStack(Object root) {
        if (!(root instanceof IInventory inventory)) return null;
    ...
    private static ItemStack findGenericStack(Object root) {
        ...
        for (Class<?> type = root.getClass(); type != null && type != Object.class; type = type.getSuperclass()) {
            for (Field field : type.getDeclaredFields()) {
                ...
                if (name.contains("pattern") && Map.class.isAssignableFrom(field.getType())) {
                    continue;
                }

                if (!isInterestingStorageField(field, name)) {
                    continue;
                }
    ...
    private static ItemStack findCircuit(Object value, IdentityHashMap<Object, Boolean> visited, int depth,
        ...
        if (value instanceof ItemStack) {
            return isIntegratedCircuit((ItemStack) value) ? (ItemStack) value : null;
        }
        ...
        if (value instanceof IInventory inventory) {
            int count;
            try {
                count = Math.min(inventory.getSizeInventory(), MAX_SLOTS);
            } catch (RuntimeException ignored) {
        ...
        if (!inspectNamedContainers || !isInspectableContainer(valueType)) {
            return null;
        }
    ...
    private static ItemStack readStackHandler(Object value, IdentityHashMap<Object, Boolean> visited, int depth,
        ...
        try {
            ...
            if (!(slotCount instanceof Number number)) {
                return null;
            }

            int count = Math.min(number.intValue(), MAX_SLOTS);
            for (int slot = 0; slot < count; slot++) {
                ItemStack result = findCircuit(
    ...
    private static Integer asConfig(Object value) {
        ...
        int config = number.intValue();
        return config >= 0 && config <= MAX_CIRCUIT_CONFIG ? config : null;
    }
